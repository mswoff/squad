...
class Final_Model(nn.Module):
    """Baseline + self-attention + character CNN + PointNet.
    ...
    def __init__(self, word_vectors, hidden_size, char_vectors, drop_prob=0.):
        ...
        self.self_att = layers.SelfAttention(hidden_size=4 * hidden_size,
                                            drop_prob=drop_prob)
...
class BiDAF_Char_XL(nn.Module):
    """Baseline + Char CNN but with larger hidden states and more fitlers for char.
    ...
    """
    def __init__(self, word_vectors, char_vectors, hidden_size, drop_prob=0.):
        super(BiDAF_Char_XL, self).__init__()
        self.emb = layers.Char_Embedding(word_vectors=word_vectors,
                                    char_vectors=char_vectors,
                                    hidden_size=hidden_size,
                                    drop_prob=drop_prob,
...
class MultiReaderBiDAF(nn.Module):
    """Baseline BiDAF model for SQuAD.
    ...
    def __init__(self, word_vectors, hidden_size, drop_prob=0.):
        ...
        self.q_encoder = layers.RNNEncoder(input_size=hidden_size,
                                     hidden_size=hidden_size,
                                     num_layers=1,
                                     drop_prob=drop_prob)

        self.c_encoder = layers.RNNEncoder(input_size=hidden_size,
                                     hidden_size=hidden_size,
                                     num_layers=1,
                                     drop_prob=drop_prob)
    ...
    def forward(self, cw_idxs, qw_idxs):
        ...
        c_enc = self.c_encoder(c_emb, c_len)    # (batch_size, c_len, 2 * hidden_size)
        q_enc = self.q_encoder(q_emb, q_len)    # (batch_size, q_len, 2 * hidden_size)
